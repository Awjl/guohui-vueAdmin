<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input style="width: 150px;" class="filter-item" placeholder="请输入商品名称" v-model="data.name"></el-input>
      <el-input style="width: 150px;" class="filter-item" placeholder="请输入订单号" v-model="data.code"></el-input>
      <el-input
        style="width: 150px;"
        class="filter-item"
        placeholder="请输入手机号"
      ...
      <el-button
        ...
        icon="document"
        @click="downloadexcel"
        :disabled="!(orderData.indexOf('1') !== -1)"
        :title="(orderData.indexOf('1') !== -1) ? '' : '暂无权限'"
      >导出 excel</el-button>
      <div class="he20"></div>
      <el-table :data="tableData" border style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="购买日期" align="center">
          <template
            slot-scope="scope"
          >{{new Date(scope.row.createDate).getFullYear()+ '-' + (((new Date(scope.row.createDate).getMonth() + 1) < 10) ? '0'+ (new Date(scope.row.createDate).getMonth() + 1) : (new Date(scope.row.createDate).getMonth() + 1)) +'-' + ((new Date(scope.row.createDate).getDate() < 10) ? '0' + new Date(scope.row.createDate).getDate() : new Date(scope.row.createDate).getDate())}}</template>
        </el-table-column>
        <el-table-column label="总价(单位：元)" align="center">
          <template
            slot-scope="scope"
          >{{((Number(scope.row.total)+Number(scope.row.price))/ 100).toFixed(2)}}</template>
        </el-table-column>
